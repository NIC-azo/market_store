...
class ProductsController {
    static getAll = errorHandler(async (req: Request, res: Response) => {
        ...
        if (results === undefined){
            return res.status(500).json({error: true, message: "error interno al obtener productos"})
        }
    ...
    static getOne = errorHandler(async (req: Request, res: Response) => {
        ...
        if (!String(id_product!) || id_product === undefined) {
            return res.status(400).json({error: true, message: "se requiere identificacion del producto, intentelo denuevo"});
        }
        const results = await productsModel.getProduct(String(id_product!));
        if (results === undefined) {
            return res.status(500).json({error: true, message: "error interno al obtener producto"});
        }
        return res.status(200).json({data: results});
    });
    static create = errorHandler(async (req: Request, res: Response) => {
        ...
        if (Object.keys(restOfBody).length <= 0) {
            return res.status(400).json({error: true, message: "no se encontraron datos, intentelo denuevo"});
        }
        if (restOfBody.bars_code) {
            ...
            if (validateExists.length > 0) {
                return res.status(409).json({error: true, message: "ya existe un producto con el mismo codigo de barras, intentelo denuevo"});
            }
        }
        ...
        if (productCreated === undefined) {
            return res.status(500).json({error: true, message: "error interno al crear producto"});
        }
    ...
    static update = errorHandler(async (req: Request, res: Response) => {
        ...
        if (id_product === undefined || !String(id_product!)) {
            return res.status(400).json({error: true, message: "identificacion del producto no valido, intentelo denuevo"});
        }
        if (Object.keys(restOfBody).length <= 0 && ((expiration_date === undefined || !expiration_date) && (production_date === undefined || !production_date))){
            return res.status(400).json({error: true, message: "se necesita almenos 1 valor para actualizar, intentelo denuevo"});
        }
        if (restOfBody.bars_code) {
            const validateExists = await productsModel.getProducts(String(restOfBody.bars_code!));
            if (validateExists.length > 0 && validateExists.some(e => e.id !== String(id_product))) {
                return res.status(409).json({error: true, message: "ya existe un producto con el mismo codigo de barras, intentelo denuevo"});
            }
        }
        ...
        if (productUpdated === undefined) {
            return res.status(500).json({error: true, message: "error interno al actualizar producto"});
        }
    ...
    static delete = errorHandler(async(req: Request, res: Response) => {
        const {id_product} = req.params;
        if (id_product === undefined || !String(id_product!)) {
            return res.status(400).json({error: true, message: "identificacion no valida del producto, intentelo denuevo"});
        }
        const productDeleted = await productsModel.deleteProduct(String(id_product!));
        if (productDeleted === undefined) {
            return res.status(500).json({error: true, message: "error interno al eliminar producto"});
        }
    ...
    static getAllerts = errorHandler(async (req: Request, res: Response) => {
        ...
        if (allerts === undefined) {
            return res.status(500).json({error: true, message: "error interno al obtener alertas"});
        }
        if (allertsExpiration === undefined) {
            return res.status(500).json({error: true, message: "error interno al obtener alertas de expiracion"});
        }
